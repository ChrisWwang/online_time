...
object OtherDayVehicleSQLdongtaichuancan {

  //TODO 测试给sql传递动态参数---------------------
  val num  = GlobalConfigUtils.num
//TODO ---------------------------------------------
  ...
  lazy val order_info_filter =
    """
      ...
      |) tb
      |where
      |DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),TO_DATE(close_gps_time)) = """+ num +"""
      |or DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),TO_DATE(close_gps_time)) = """+ (num - 1) +"""
      |or DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),TO_DATE(begin_time)) = """+ num +"""
      |or DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),TO_DATE(begin_time)) = """+ (num + 1) +"""
    """.stripMargin
  ...
  lazy val driver_online_record_nextday =
    """
      ...
      |driver_id ,
      |time,
      |UNIX_TIMESTAMP(date_sub(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),"""+ (num - 1) +"""),'yyyy-MM-dd') - time as online_time
      |from
...
//    """.stripMargin
  ...
  lazy val order_info_yesterday =
    """
      ...
      |order_info_filter
      |where DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),close_gps_time) = """+ num +"""
      |and DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),begin_time) = """+ (num + 1) +"""
    """.stripMargin
  ...
  lazy val order_info_nextday =
    """
      |select
      |driver_id,
      |UNIX_TIMESTAMP(date_sub(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),"""+ (num - 1) +"""),'yyyy-MM-dd') - begin_time_s as online_time
      |from
      |order_info_filter
      |where DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),close_gps_time) = """+ (num - 1) +"""
      |and DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),begin_time) = """+ num +"""
    """.stripMargin
