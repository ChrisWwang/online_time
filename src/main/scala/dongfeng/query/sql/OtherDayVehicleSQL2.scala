...
object OtherDayVehicleSQL2 {
  ...
  lazy val order_info_sameday =
    """
      |select
      |driver_id,
      |UNIX_TIMESTAMP(close_gps_time) - UNIX_TIMESTAMP(begin_time) as online_time
      |from
      |order_info
      |where DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),TO_DATE(close_gps_time)) = 2
      |and DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),TO_DATE(begin_time)) = 2
      |and UNIX_TIMESTAMP(close_gps_time) - UNIX_TIMESTAMP(begin_time) > 0
      |and driver_id < 2000
    """.stripMargin
  ...
  lazy val order_info_yesterday =
    """
      ...
      |order_info
      |where DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),TO_DATE(close_gps_time)) = 2
      |and DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),TO_DATE(begin_time)) = 1
      |and driver_id < 2000
    """.stripMargin
  ...
  lazy val order_info_nextday =
    """
      |select
      |driver_id,
      |UNIX_TIMESTAMP(date_sub(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),1),'yyyy-MM-dd') - UNIX_TIMESTAMP(begin_time) as online_time
      |from
      |order_info
      |where DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),TO_DATE(close_gps_time)) = 1
      |and DATEDIFF(from_unixtime(unix_timestamp(),'yyyy-MM-dd'),TO_DATE(begin_time)) = 2
